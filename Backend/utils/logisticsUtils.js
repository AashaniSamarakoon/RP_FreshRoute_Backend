...
// Standard Haversine formula for straight-line distance (Fallback & Proximity Scoring)
function calculateDistanceKm(lat1, lon1, lat2, lon2) {
  if (!lat1 || !lon1 || !lat2 || !lon2) return 0;
  const R = 6371;
  const dLat = ((lat2 - lat1) * Math.PI) / 180;
...
// REAL-WORLD FORECAST (OpenWeather API) - Scans next 24 hours for daily planning
async function getRealWeather(lat, lng) {
  try {
    if (!lat || !lng) throw new Error("Missing coordinates");
    const apiKey = process.env.OPENWEATHER_API_KEY;

    // Uses the forecast endpoint to ensure we don't accidentally check cold midnight temperatures
    ...
    // Extract the next 8 periods (3 hours * 8 = 24 hours)
    const next24Hrs = response.data.list.slice(0, 8);

    // Find the maximum forecasted temperature for the day
    const maxTempC = Math.max(
      ...next24Hrs.map((period) => period.main.temp_max),
    );

    // Check if any period in the next 24 hours has rain (Codes 200-599)
    const isRaining = next24Hrs.some((period) => {
      const conditionId = period.weather[0].id;
    ...
    console.log(
      "[WEATHER FALLBACK] Failed to fetch forecast, defaulting to 30C / No Rain",
    );
    return { temp_c: 30, raining: false, condition: "Unknown (Fallback)" };
  }
}
